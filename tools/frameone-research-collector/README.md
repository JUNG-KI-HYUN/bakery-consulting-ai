# FRAMEONE Research Collector V1.3.2
...
코드를 갱신한 경우 `chrome://extensions`에서 이 확장프로그램의 새로고침 버튼을 눌러야 manifest의 `storage` 권한과 V1.3.2 스크립트가 적용됩니다.
...
- 같은 scope의 `월임대료 B만원` 또는 `월세 B만원`이 헤더 B와 일치해도 월세 단위를 자동확인하며, 불일치하면 확인필요를 유지합니다.
- 건물 총주차와 임차조건의 무료 제공 주차를 분리하고, 추가 주차 협의는 별도 observation으로 보존합니다.
- 건물명은 명시적 라벨 또는 현재 매물 제목·설명에서 동일한 건물 고유명이 반복될 때만 자동확인합니다.
